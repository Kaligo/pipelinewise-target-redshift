...
class TestFastSyncLoader:
    ...
    def _create_loader(self, config=None, schema=None, mock_connect=None, mock_boto3=None):
    ...
    def _get_sql_calls(self, mock_cursor, sql_keyword):
        ...
        return [call for call in mock_cursor.execute.call_args_list
    ...
    def test_fast_sync_with_multiple_files(self, mock_connect, mock_boto3):
        ...
        assert mock_s3_client.delete_object.call_count >= 3, "Should delete at least 3 files"
    ...
    def test_fast_sync_deletion_detection(self, mock_connect, mock_boto3):
        ...
        loader, mock_cursor, _ = self._create_loader(
            config=config_with_metadata, mock_connect=mock_connect, mock_boto3=mock_boto3
        )
    ...
    def test_fast_sync_insert_update(self, mock_connect, mock_boto3):
        ...
        all_calls = self._get_sql_strings(mock_cursor)
        update_calls = [call_str for call_str in all_calls
                       if call_str.strip().startswith('UPDATE') and 'FROM' in call_str]
        insert_calls = [call_str for call_str in all_calls
    ...
    def test_fast_sync_full_refresh(self, mock_connect, mock_boto3):
        ...
        all_calls = self._get_sql_strings(mock_cursor)
        swap_calls = [call_str for call_str in all_calls
    ...
    def test_fast_sync_copy_options_adds_region_when_missing(self, mock_connect):
        ...
        assert "REGION 'ap-southeast-1'" in copy_sql, "Should add REGION when not present"
    ...
    def test_fast_sync_deletion_detection_no_primary_key_full_table(self, mock_connect, mock_boto3):
        ...
        loader, mock_cursor, _ = self._create_loader(
            config=config_with_metadata, schema=schema_no_pk,
            mock_connect=mock_connect, mock_boto3=mock_boto3
        )
