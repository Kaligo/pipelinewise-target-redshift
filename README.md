...
| skip_unchanged_rows                 | Boolean |    No      | (Default: True) When enabled, UPDATE and INSERT operations will skip rows where all non-metadata column values are identical between the source and target tables. This optimization prevents unnecessary DELETE+INSERT operations in Redshift when data hasn't changed, improving performance. Metadata columns (`_SDC_EXTRACTED_AT`, `_SDC_BATCHED_AT`, `_SDC_DELETED_AT`) are excluded from comparison as they change on every sync. When disabled, all matching rows will be updated/inserted regardless of whether values have changed. |
